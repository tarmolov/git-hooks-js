        ...
        if (fsHelpers.exists(hooksDirname)) {
            ...
            var hooks = list
                ...
                .filter(function (hookPath) {
                    if (!isExecutable(hookPath) && !isJS(hookPath)) {
...
function isExecutable(file) {
    return fs.lstatSync(file).isFile() && fsHelpers.isExecutable(file);
}

function isJS(file) {
    return fs.lstatSync(file).isFile() && path.extname(file) === '.js';
}
